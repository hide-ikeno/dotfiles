return {
  ...
  },
  {
    ...
    setup = function()
      ...
      vim.api
        .nvim_set_keymap("x", "sr", "<Plug>(operator-sandwich-replace)", {})
  ...
  -- SKK input method for Japanese
  {
    ...
    config = function()
      ...
      elseif vim.fn.has('win32') or vim.fn.has('win64') then
        vim.g["eskk#large_dictionary"] =
          { path = "~/SKK-JISYO.L", sorted = 1, encoding = "euc-jp" }
      else
        vim.g["eskk#large_dictionary"] =
          {
      ...
      -- Key mappings
      vim.api.nvim_set_keymap(
        "i", "<C-j>", "<Plug>(eskk:toggle)", { silent = true }
      )
      vim.api.nvim_set_keymap(
        "c", "<C-j>", "<Plug>(eskk:toggle)", { silent = true }
      )
