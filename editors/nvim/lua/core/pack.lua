...
function Packer:load_packer()
  ...
  require(
    ...
    packer.use, packer.use_rocks
  )
  -- local plugin_config_files = vim.split(
  --   vim.fn.glob(plugin_config_dir .. "/*.lua"), "\n"
  -- )
...
function Packer:init_ensure_plugins()
  ...
  local state = vim.loop.fs_stat(
    packer_dir
  )
  if not state then
    local out = vim.fn.system(
      string.format(
        'git clone %s %s', packer_repo, packer_dir
      )
    )
    print(
      out
...
function plugins.auto_compile()
  ...
  if file:match(
    "nvim/lua/plugins/%a+.lua"
  ) then plugins.compile() end
end
